...
class P05_11_LongestAlternatingSubsequence {
     ...
     * @return
     */
    public int findLASLengthV1(int[] nums) {
        Integer[][][] dp = new Integer[nums.length][nums.length][2];
        return Math.max(findLASLengthRecursiveV1(dp, nums, -1, 0, true),
                findLASLengthRecursiveV1(dp, nums, -1, 0, false));
    }

    private int findLASLengthRecursiveV1(Integer[][][] dp, int[] nums,
        ...
        if(dp[previousIndex+1][currentIndex][isAsc ? 1 : 0] == null) {
            ...
            if(isAsc) {
                if(previousIndex == -1 || nums[previousIndex] < nums[currentIndex])
                    c1 = 1 + this.findLASLengthRecursiveV1(dp, nums, currentIndex, currentIndex+1, !isAsc);
            } else { // if descending, the next element should be smaller
                if(previousIndex == -1 || nums[previousIndex] > nums[currentIndex])
                    c1 = 1 + this.findLASLengthRecursiveV1(dp, nums, currentIndex, currentIndex+1, !isAsc);
            }
            // skip the current element
            int c2 = this.findLASLengthRecursiveV1(dp, nums, previousIndex, currentIndex+1, isAsc);
     ...
     * @return
     */
    public int findLASLengthV2(int[] nums) {
    ...
    public static void main(String[] args) {
        P05_11_LongestAlternatingSubsequence las = new P05_11_LongestAlternatingSubsequence();
        int[] nums = {1,2,3,4};
